...
# 1. Invariant Normalize : Support ports (List) et port (Legacy)
@pytest.mark.asyncio
async def test_barrier_normalize_target():
    t = await normalize_target("http://example.com:8080")
    # normalize_target renvoie désormais un objet TargetV1
    assert 8080 in t.ports
    assert t.scheme == "http"
    assert t.port == 8080


# 2. Invariant Engine : Vérifie la capture propre des exceptions
@pytest.mark.asyncio
...
# 3. Invariant Signals : Protection contre None
...
# 4. Invariant CMS : Résistance aux entrées corrompues
...
# 6. Invariant Probe : Signature Variante A (DI)
@pytest.mark.asyncio
async def test_barrier_probe_di_logic():
    ...
    mock_resp.headers = {}
    mock_resp.aiter_bytes = AsyncMock(return_value=[b"ok"])
